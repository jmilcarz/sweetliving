...
class auth {
     ...
     public static $system_login_with_email_or_phone = false;
     static function logout() {
          DB::query('DELETE FROM login_tokens WHERE login_tokens_user_id=:userid', array(':userid'=>self::loggedin()));
     ...
     public static function loggedin() {
          if (isset($_COOKIE['' . self::$system_cookie_name . ''])) {
               if (DB::query('SELECT login_tokens_user_id FROM login_tokens WHERE login_tokens_token=:token', array(':token'=>sha1($_COOKIE['' . self::$system_cookie_name . ''])))) {
                    $userid = DB::query('SELECT login_tokens_user_id FROM login_tokens WHERE login_tokens_token=:token', array(':token'=>sha1($_COOKIE['' . self::$system_cookie_name . ''])))[0]['login_tokens_user_id'];
                    if (isset($_COOKIE['' . self::$system_cookie_name . '_'])) {
                         ...
                         DB::query('DELETE FROM login_tokens WHERE login_tokens_token=:token', array(':token'=>sha1($_COOKIE["" . self::$system_cookie_name . ""])));
